jest.mock('@angular/router');

import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { FormBuilder } from '@angular/forms';
import { ActivatedRoute } from '@angular/router';
import { of, Subject } from 'rxjs';

import { SquadService } from '../service/squad.service';
import { ISquad, Squad } from '../squad.model';

import { SquadUpdateComponent } from './squad-update.component';

describe('Component Tests', () => {
  describe('Squad Management Update Component', () => {
    let comp: SquadUpdateComponent;
    let fixture: ComponentFixture<SquadUpdateComponent>;
    let activatedRoute: ActivatedRoute;
    let squadService: SquadService;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        declarations: [SquadUpdateComponent],
        providers: [FormBuilder, ActivatedRoute],
      })
        .overrideTemplate(SquadUpdateComponent, '')
        .compileComponents();

      fixture = TestBed.createComponent(SquadUpdateComponent);
      activatedRoute = TestBed.inject(ActivatedRoute);
      squadService = TestBed.inject(SquadService);

      comp = fixture.componentInstance;
    });

    describe('ngOnInit', () => {
      it('Should update editForm', () => {
        const squad: ISquad = { id: 456 };

        activatedRoute.data = of({ squad });
        comp.ngOnInit();

        expect(comp.editForm.value).toEqual(expect.objectContaining(squad));
      });
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', () => {
        // GIVEN
        const saveSubject = new Subject();
        const squad = { id: 123 };
        spyOn(squadService, 'update').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ squad });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.next(new HttpResponse({ body: squad }));
        saveSubject.complete();

        // THEN
        expect(comp.previousState).toHaveBeenCalled();
        expect(squadService.update).toHaveBeenCalledWith(squad);
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', () => {
        // GIVEN
        const saveSubject = new Subject();
        const squad = new Squad();
        spyOn(squadService, 'create').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ squad });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.next(new HttpResponse({ body: squad }));
        saveSubject.complete();

        // THEN
        expect(squadService.create).toHaveBeenCalledWith(squad);
        expect(comp.isSaving).toEqual(false);
        expect(comp.previousState).toHaveBeenCalled();
      });

      it('Should set isSaving to false on error', () => {
        // GIVEN
        const saveSubject = new Subject();
        const squad = { id: 123 };
        spyOn(squadService, 'update').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ squad });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.error('This is an error!');

        // THEN
        expect(squadService.update).toHaveBeenCalledWith(squad);
        expect(comp.isSaving).toEqual(false);
        expect(comp.previousState).not.toHaveBeenCalled();
      });
    });
  });
});
